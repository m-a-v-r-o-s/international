...
describe('parseBulkPaste', () => {
  test('reads a tab-separated block, category codes upper-cased', () => {
    const result = parseBulkPaste(
      'a\t35.00\t65.00\t90.00\t115.00\t140.00\t160.00\t180.00\t25.00\n' +
    ...
    expect(result.rows[0]).toEqual({
      ...
      cents: [3500, 6500, 9000, 11500, 14000, 16000, 18000, 2500],
    })
    expect(result.rows[1]!.cents).toEqual([4000, 7500, 10500, 13500, 16500, 19000, 21500, 3000])
  })

  test('accepts comma and semicolon separators, and comma decimals', () => {
    const result = parseBulkPaste('A,35,65,90,115,140,160,180,25', CODES)
    expect(result.ok).toBe(true)
    const semi = parseBulkPaste('A;35,50;65;90;115;140;160;180;25', CODES)
    expect(semi.ok).toBe(true)
    if (!semi.ok) return
    expect(semi.rows[0]!.cents[0]).toBe(3550)
  })
