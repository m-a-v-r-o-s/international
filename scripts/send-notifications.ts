 ...
 *
 *   npm run notify -- --exceptions   the boss's inbox: anything not yet told
 *   npm run notify -- --morning      each rep's own pickups today
 *   npm run notify -- --evening      each rep's own returns due today
 *
 * On Railway these are cron services against the same project. The exceptions
 * sweep wants a short interval (every few minutes) because damage flagged at a
 ...
 */
import {
  notifyEveningReturns, notifyMorningPickups, notifyPendingExceptions,
} from '../src/lib/push/notify'

const modes = ['exceptions', 'morning', 'evening'] as const
...
const outcome = mode === 'exceptions'
  ? await notifyPendingExceptions()
  : mode === 'morning'
...
if ('announced' in outcome) console.log(`  exceptions marked as told: ${outcome.announced}`)
